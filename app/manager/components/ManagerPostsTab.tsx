...
import PostCard from '@/app/smm/components/PostCard'
...
function ClientFilterDropdown({ clients, selected, onChange }: {
  clients: Client[]
  selected: string[]
            ...
            <label key={c.id} className="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer">
              ...
              <div className="flex items-center gap-2 flex-1">
                {getLogoUrl(c.logo_path) ? (
                  <img src={getLogoUrl(c.logo_path)!} alt={c.name} className="w-5 h-5 rounded-full object-cover flex-shrink-0" />
                ) : (
                  <div className="w-5 h-5 rounded-full bg-[#10375C]/10 flex items-center justify-center text-[#10375C] text-xs font-bold flex-shrink-0">
                    {c.name[0]}
                  </div>
                )}
...
export default function ManagerPostsTab({ initialPosts, clients }: ManagerPostsTabProps) {
  ...
  const filteredPosts = posts.filter(post => {
    ...
    if (periodRange && post.scheduled_date) {
      if (!isWithinInterval(parseISO(post.scheduled_date), periodRange)) return false
    }
    return true
  })
